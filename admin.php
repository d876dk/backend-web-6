...
$db_user = 'u52995';
$db_pass = '1306430';
$db = new PDO('mysql:host=localhost;dbname=u52995', $db_user, $db_pass, array(
...
function show_tables($db)
{
  $sql = 'SELECT  applications.*, 
            ...
            FROM applications
            INNER JOIN supersss
                ON applications.id = supersss.id
            INNER JOIN superdef 
                ON supersss.superpowers = superdef.id
            INNER JOIN usersss 
                ON usersss.id = applications.id;';
?>
  <table class="table">
    ...
    {
      try {
        $sth = $db->prepare("DELETE FROM applications WHERE id = ?");
        $sth->execute(array($del));
        $sth = $db->prepare("DELETE FROM supersss WHERE id = ?");
        $sth->execute(array($del));
        $sth = $db->prepare("DELETE FROM usersss WHERE id = ?");
        $sth->execute(array($del));
    ...
    function add_user($db)
    {
      ...
      try {
        ...
        $id_db = $db->lastInsertId("applications");
    ...
    <?php
      ...
      try {
        if (!empty($_GET['delete_id'])) {
          delete_user($db, $_GET['delete_id']);
        }
        if (!empty($_GET['edit_id'])) if (!errors()) {
          edit_user($db, $_GET['edit_id']);
        }
        if (isset($_GET['act']) && $_GET['act'] == 'add_article') {
          add_user($db);
        }
      } catch (PDOException $e) {
