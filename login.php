...
function change_pass($db)
{
  ...
  $stmt = $db->prepare("SELECT * FROM usersss WHERE login = ?");
  $stmt->execute(array(
    $login_ch
  ));
  $user = $stmt->fetch();
  if (password_verify($old_pass, $user['pass'])) {
    $stmt = $db->prepare("UPDATE usersss SET pass = ? WHERE login = ?");
    $stmt->execute(array(
...
?>
    <form action="" method="POST">
      <h2> Здесь Вы можете изменить пароль: </h2>
